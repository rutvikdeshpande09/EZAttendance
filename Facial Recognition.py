...
def process_frame(frame):
    ...
    for i, face_encoding in enumerate(face_encodings):
        ...
        # Use stricter tolerance (lower value = stricter matching)
        # Default is 0.6, we'll use 0.5 for better accuracy
        tolerance = 0.5
        
        # Check if the best match is good enough
        if best_distance <= tolerance:
            # Additional validation: check if there's a clear winner
            # If the second-best match is too close, it might be ambiguous
            if len(face_distances) > 1:
                sorted_distances = np.sort(face_distances)
                second_best_distance = sorted_distances[1]
                # If the gap between best and second best is too small, mark as Unknown
                if second_best_distance - best_distance < 0.1:
                    name = "Unknown"  # Too ambiguous, don't match
                else:
